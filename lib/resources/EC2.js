  ...
  describe() {
    ...
    return co(function*() {
      return {
        ...
        // flatten Instaces from Reservations
        Instances: (yield self.resource.describeInstances().promise()).data.Reservations.map(function(x) { return x.Instances.map(Utility.addACIKey("EC2_Instances", "InstanceId")); }).reduce(function(x, y) { return x.concat(y); }),
